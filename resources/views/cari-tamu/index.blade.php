...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div id="resultsContainer">
        @include('cari-tamu._guest-list', ['guests' => $guests])
    </div>
</div>

{{-- Modal Check-in --}}
@include('cari-tamu._checkin-modal')
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const showModal = (guestId, guestName) => {
        if (!modal || !checkInForm || !guestNameEl) return;
    ...
    // 1. Listener untuk input pencarian
    searchInput.addEventListener('keyup', debounce(performSearch, 300));

    // 2. Listener untuk tombol check-in (menggunakan event delegation)
    document.body.addEventListener('click', function(event) {
        if (event.target.matches('.action-checkin')) {
