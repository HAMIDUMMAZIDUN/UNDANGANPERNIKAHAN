    ...
    <div class="relative z-10 flex flex-col justify-center items-center h-full w-full">
        ...
        <div class="w-full sm:max-w-md px-8 py-10 sm:rounded-[30px] relative overflow-hidden glass-shadow
            ...
            <div class="text-center mb-8 relative">
                <h2 class="font-script text-5xl text-white drop-shadow-md mb-1">Welcome</h2>
                <p class="font-sans-serif text-[10px] uppercase tracking-[4px] text-white/90 font-bold">Please Login to Continue</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="mb-5">
                    <label for="email" class="block font-sans-serif text-[11px] font-bold uppercase tracking-wider text-white/90 mb-2 ml-1">Email Address</label>
                    <input id="email" 
                ...
                <div class="flex items-center justify-between mb-8">
                    <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                        <input id="remember_me" type="checkbox" class="rounded border-white/40 bg-white/20 text-[#15803d] shadow-sm focus:ring-[#15803d] transition ease-in-out duration-150" name="remember">
                        <span class="ms-2 text-xs text-white/90 group-hover:text-white font-sans-serif transition-colors">{{ __('Remember me') }}</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-xs text-white/80 hover:text-white font-sans-serif font-semibold transition-colors duration-200 border-b border-transparent hover:border-white pb-0.5" href="{{ route('password.request') }}">
                            {{ __('Forgot password?') }}
                        </a>
                    @endif
                </div>
                ...
                <div class="mt-6 text-center">
                    <p class="text-xs text-white/80 font-sans-serif">
                        Don't have an account? 
                        <a href="{{ route('register') }}" class="text-[#86efac] font-bold hover:text-[#4ade80] hover:underline transition-all ml-1">Sign Up</a>
                    </p>
                </div>
            </form>
        </div>
