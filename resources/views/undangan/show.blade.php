    ...
    <main id="main-content" class="relative z-10 bg-brand-cream">
        ...
        <div align="center" class="max-w-md mx-auto" data-aos="fade-up">
            ...
            <div class="mt-8 bg-white p-6 inline-block rounded-2xl shadow-lg">
                {{-- PERBAIKAN: QR Code ini berisi URL unik tamu untuk check-in OTOMATIS --}}
                {!! QrCode::size(250)->generate(route('undangan.show', ['event' => $event, 'guest' => $guest])) !!}
            </div>
            <p class="mt-4 font-semibold text-brand-dark">{{ $guest->name }}</p>
        </div>
