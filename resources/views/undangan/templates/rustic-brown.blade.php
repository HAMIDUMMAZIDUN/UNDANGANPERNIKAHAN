...
<body class="antialiased overflow-x-hidden">
    ...
    <div id="cover" class="h-screen w-full flex flex-col justify-center items-center text-center p-4 bg-brand-light relative overflow-hidden">
        ...
        <div class="relative z-20" data-aos="zoom-in">
            ...
            <div class="my-6 relative w-48 h-48 mx-auto p-2 bg-white/50 shadow-lg rounded-md">
                <img src="{{ $event->photo_url ? (Str::startsWith($event->photo_url, 'http') ? $event->photo_url : asset('storage/' . $event->photo_url)) : 'https://i.ibb.co/VMyB8fN/story-1-AA86-E7-C3.jpg' }}" class="w-full h-full object-cover rounded-sm">
            </div>
        ...
        <div class="max-w-3xl mx-auto bg-brand-bg shadow-2xl relative">
                ...
                <div class="relative z-10">
                    ...
                    <div class="mt-12 flex flex-col items-center gap-8">
                        <div class="text-center">
                            ...
                            <h3 class="font-title text-4xl mt-4 text-brand-gold">{{ $event->bride_name }}</h3>
                        ...
                        <div class="text-center">
                            ...
                            <h3 class="font-title text-4xl mt-4 text-brand-gold">{{ $event->groom_name }}</h3>
            ...
            <section id="qr-code" class="py-20 px-6 text-center">
                <div class="max-w-md mx-auto" data-aos="fade-up">
                    <h2 class="font-title text-5xl text-brand-gold">QR Code Kehadiran</h2>
            ...
            <section id="ucapan" class="py-20 px-6 relative">
                ...
                <div class="relative z-10">
                    ...
                    <div class="max-w-md mx-auto p-6 bg-brand-light @if(!empty($event->gifts) && $event->gifts->isNotEmpty()) -mt-10 @endif relative rounded-2xl shadow-lg">
                        ...
                        <div class="mt-8" data-aos="fade-up" data-aos-delay="200">
                            ...
                            <form action="{{ route('rsvp.store', $event->uuid) }}" method="POST">
                                @csrf
                                <div class="mb-4">
                                    <input type="text" id="name" name="name" class="block w-full border-brand-gold/50 rounded-md shadow-sm bg-white/50 p-3 @if(isset($guest->uuid) && $guest->uuid !== 'umum') bg-gray-100 text-gray-500 @endif" value="{{ $guest->name ?? old('name') }}" required @if(isset($guest->uuid) && $guest->uuid !== 'umum') readonly @endif placeholder="Nama Anda">
                                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div class="mb-4">
                                    <textarea id="message" name="message" rows="4" class="block w-full border-brand-gold/50 rounded-md shadow-sm bg-white/50 p-3" placeholder="Tulis ucapan..." required>{{ old('message') }}</textarea>
    ...
    <div id="gift-modal" class="fixed inset-0 bg-black/60 z-[1000] flex justify-center items-center p-4 hidden">
        <div id="gift-modal-content" class="bg-brand-light rounded-lg p-6 w-full max-w-sm text-center relative transform scale-95 transition-transform duration-300">
            ...
            <div class="space-y-4 text-left">
                ...
                <div class="bg-white/50 p-4 rounded-lg" data-account="{{ $gift->account_number }}">
    ...
    <nav class="bottom-nav fixed bottom-0 left-0 w-full bg-white/80 backdrop-blur-md shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-[500] hidden">
        <div class="max-w-3xl mx-auto flex justify-around text-brand-green/80 text-center py-3">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (statusInput && statusInput.value) {
            document.querySelector(`.rsvp-btn[data-value="${statusInput.value}"]`)?.classList.add('bg-brand-green', 'text-white');
        }
        rsvpButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                rsvpButtons.forEach(btn => {
                    ...
                    btn.classList.add('hover:bg-brand-green/10');
                });
                button.classList.add('bg-brand-green', 'text-white');
                button.classList.remove('hover:bg-brand-green/10');
            });
        });
