...
<div class="flower-decoration fixed bottom-0 right-0 translate-x-1/4 translate-y-1/4 z-[1] pointer-events-none opacity-0 scale-75 transition-all duration-1000 ease-in-out">
    ...
    <audio id="background-music" loop> <source src="{{ asset('music/musik1.mp3') }}" type="audio/mpeg"> </audio>
    ...
    <div id="cover" class="h-screen w-full flex flex-col justify-center items-center text-center p-4 bg-cover bg-center" style="background-image: url('{{ $event->photo_url ? asset('storage/' . $event->photo_url) : 'https://images.unsplash.com/photo-1532703108233-63d1ceb56a6c?auto=format&fit=crop&w=987' }}');">
        <div class="absolute inset-0 bg-black/40"></div>
        <div class="relative z-10 w-full max-w-sm sm:max-w-md bg-white/90 text-brand-dark p-8 sm:p-12 rounded-t-[100px] rounded-b-[40px] shadow-2xl border-4 border-white/50" data-aos="fade-up">
            ...
            @endphp
            <div class="font-playfair text-8xl sm:text-9xl font-bold my-4 text-brand-dark/90">
            ...
            <button id="open-invitation" class="mt-8 bg-brand-dark text-white font-bold py-3 px-8 rounded-full shadow-lg hover:bg-slate-700 transition-all duration-300 hover:scale-105 flex items-center gap-2 mx-auto">
    ...
    <main id="main-content" class="relative z-10 bg-brand-cream">
        ...
        <section class="py-24 px-4 text-center">
             <div class="max-w-5xl mx-auto">
                <h2 class="font-playfair text-5xl font-bold text-brand-dark" data-aos="fade-up">Mempelai</h2>
                <div class="mt-16 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                    <div data-aos="fade-right" data-aos-delay="200">
                        <img src="{{ $event->groom_photo ? asset('storage/' . $event->groom_photo) : 'https://placehold.co/400x400/e2e8f0/64748b?text=Groom' }}" class="w-48 h-48 rounded-full object-cover mx-auto border-4 border-brand-gold shadow-lg">
                        ...
                        @if($event->groom_instagram)
                            <a href="https://instagram.com/{{ str_replace('@', '', $event->groom_instagram) }}" target="_blank" class="inline-block mt-2 text-sm text-brand-gold hover:underline">{{ '@' . str_replace('@', '', $event->groom_instagram) }}</a>
                        @endif
                    </div>
                    <div data-aos="fade-left" data-aos-delay="400">
                         <img src="{{ $event->bride_photo ? asset('storage/' . $event->bride_photo) : 'https://placehold.co/400x400/e2e8f0/64748b?text=Bride' }}" class="w-48 h-48 rounded-full object-cover mx-auto border-4 border-brand-gold shadow-lg">
                        ...
                        @if($event->bride_instagram)
                            <a href="https://instagram.com/{{ str_replace('@', '', $event->bride_instagram) }}" target="_blank" class="inline-block mt-2 text-sm text-brand-gold hover:underline">{{ '@' . str_replace('@', '', $event->bride_instagram) }}</a>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <section class="py-24 px-4 text-center bg-brand-dark text-white relative bg-cover bg-center bg-fixed" style="background-image: url('{{ $event->photo_url ? asset('storage/' . $event->photo_url) : '' }}');">
            ...
            <div class="relative z-10 max-w-4xl mx-auto" data-aos="fade-up">
                ...
                @php
                    ...
                    $gcal_title = urlencode($event->name ?? 'Pernikahan ' . $event->groom_name . ' & ' . $event->bride_name);
        ...
        {{-- Logika untuk menampilkan QR Code hanya untuk tamu spesifik, bukan untuk umum --}}
        @if(isset($guest) && $guest->uuid)
        <section id="qrcode" class="py-24 px-4 text-center section-bg-pattern">
            <div align="center" class="max-w-md mx-auto" data-aos="fade-up">
                <h2 class="font-playfair text-5xl font-bold text-brand-dark">QR Code </h2>
                <p class="mt-4 text-slate-600">
                    Tunjukan QR Code ini ke Penjaga Acara untuk proses check-in kedatangan Anda.
                </p>
                <div class="mt-8 bg-white p-6 inline-block rounded-2xl shadow-lg">
                    {!! QrCode::size(250)->generate(route('manual.index')) !!}
                </div>
            </div>
        </section>
        @endif
...
<section id="wedding-wishes" class="py-24 px-4 text-center section-bg-pattern">
    <div class="max-w-4xl mx-auto">
         ...
         @if(session('success'))
            <div class="mt-8 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative" role="alert" data-aos="fade-up">
                 ...
                 <div class="mb-4">
                     ...
                     <div class="mt-2 space-y-2 sm:space-y-0 sm:space-x-4">
                         <label class="inline-flex items-center">
                             <input type="radio" class="form-radio text-brand-gold focus:ring-brand-gold" name="status" value="Hadir" {{ old('status') == 'Hadir' ? 'checked' : '' }} required>
                             <span class="ml-2">Hadir</span>
                         </label>
                         <label class="inline-flex items-center">
                             <input type="radio" class="form-radio text-brand-gold focus:ring-brand-gold" name="status" value="Tidak Hadir" {{ old('status') == 'Tidak Hadir' ? 'checked' : '' }}>
         ...
         @if($rsvps->hasPages())
            <div class="mt-8">{{ $rsvps->links() }}</div>
         @endif
    </div>
</section>
