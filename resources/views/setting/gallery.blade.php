        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            @forelse ($photos as $photo)
                <div class="relative group rounded-lg overflow-hidden shadow-md">
                    <img src="{{ asset('storage/' . $photo->path) }}" alt="Foto Event" class="w-full h-48 object-cover">
                     <form action="{{ route('setting.gallery.delete', $photo) }}" method="POST" class="delete-form">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-3 bg-red-600/70 rounded-full hover:bg-red-700">
