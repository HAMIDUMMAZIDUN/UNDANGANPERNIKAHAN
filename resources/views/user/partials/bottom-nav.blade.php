<div class="fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-20 md:left-64">
    <nav class="flex justify-around h-16">
        ...
        <!-- ITEM BARU: Manual Check-in -->
        <a href="{{ route('manual.index') }}" class="flex flex-col items-center justify-center w-full text-center {{ request()->is('manual-check-in*') ? 'text-blue-600' : 'text-slate-500' }}">
            <i class="fas fa-pen-to-square text-xl"></i>
            <span class="text-xs font-medium mt-1">Manual</span>
        </a>
    </nav>
</div>
