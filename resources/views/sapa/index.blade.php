...
<head>
    ...
    <title>Layar Sapa {{ $event ? '- ' . $event->name : '' }}</title>
...
<body class="bg-slate-900 text-white overflow-hidden">
    ...
    @if ($event)
        {{-- Tampilan Slideshow jika event ditemukan --}}
    ...
    @else
        {{-- Tampilan jika TIDAK ADA event sama sekali di database --}}
        <div class="w-screen h-screen flex flex-col items-center justify-center text-center p-8">
            <h1 class="text-4xl font-bold text-amber-400">Layar Sapa</h1>
            <p class="mt-4 text-xl text-slate-300">
                Belum ada event yang dibuat di dalam sistem.
            </p>
            <p class="mt-2 text-slate-400">
                Silakan login ke dashboard dan buat event pertama Anda untuk menggunakan fitur ini.
            </p>
            <a href="{{ route('login') }}" class="mt-8 bg-amber-500 text-slate-900 font-bold py-3 px-6 rounded-lg hover:bg-amber-600 transition-colors">
                Login ke Dashboard
            </a>
        </div>
    @endif
